...
import { CarServicingBays } from "@/components/dashboard/industries/car-servicing-bays";
import { LayoutGrid, Rocket, Construction, ArrowRight } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function ActiveBaysPage() {
  const { category } = useBusiness();

  const renderBaysView = () => {
    switch (category) {
      case "Car Servicing":
        return <CarServicingBays />;
      default:
        return (
          <div className="flex min-h-[70vh] flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-700 fill-mode-both">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-zinc-900">
              Live Monitoring for {category} Coming Soon
            </h2>
            <p className="mt-3 max-w-sm text-zinc-500">
              Building the most efficient real-time bay and resource management system for the <span className="font-bold text-zinc-900">{category}</span> sector.
            </p>
